        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 justify-items-center w-full" style={{transform: "translateY(8vh)"}}>
          {loading
            ...
            : filteredThemes.map((theme) => (
                <ThemeCard key={theme.id} theme={theme} isPreviewed={previewIds.includes(theme.id)} onPreview={() => onPreview(theme.id)} />
              ))}
        </div>
      </div>
                ...
                {previewIds.length > 0 ? (
                  <ul className="divide-y divide-gray-300">
                    {previewIds.map((id, index) => (
                      <li
                        key={index}
                        className="p-3 flex items-center justify-between text-gray-700 hover:bg-gray-200 rounded transition duration-150"
                      >
                        <span>{id}</span>
                      </li>
                    ))}
